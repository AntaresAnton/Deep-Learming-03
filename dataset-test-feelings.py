    ...
    for name, dtype_series in dtypes.items():
        ...
        for col in common_columns:
            if dtype_series[col] != reference_dtypes[col]:
                print(f"❌ Column '{col}' has different data types: {reference_name}={reference_dtypes[col]}, {name}={dtype_series[col]}")
...
def check_data_order(datasets, order_column=None):
    ...
    if order_column is None:
        ...
        potential_order_columns = [col for col in common_columns if any(keyword in col.lower() for keyword in ['id', 'time', 'date', 'timestamp', 'order'])]
...
def visualize_distributions(datasets, max_columns=5):
    ...
    common_numeric_columns = set.intersection(*[set(cols) for cols in numeric_columns.values()])
...
def check_duplicates(datasets):
    ...
    for name, df in datasets.items():
        ...
        if duplicates > 0:
            print(f"⚠️ {name} dataset has {duplicates} duplicate rows ({duplicates/len(df)*100:.2f}%)")
        else:
