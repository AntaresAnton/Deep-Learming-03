...
class TextProcessor:
    def __init__(self, file_path):
        ...
        print(f"{Fore.YELLOW}Tamaño del vocabulario: {Fore.WHITE}{self.vocab_size} caracteres únicos")
    ...
    def create_sequences(self, seq_length, max_sequences=None):
        ...
        print(f"{Fore.YELLOW}Memoria aproximada utilizada: {X.nbytes / (1024**2):.2f} MB + {y.nbytes / (1024**2):.2f} MB = {(X.nbytes + y.nbytes) / (1024**2):.2f} MB{Style.RESET_ALL}")
...
class TextGenerationModel(nn.Module):
    ...
    def generate_text(self, processor, seed_text, length=100, temperature=1.0):
        ...
        print(f"{Fore.YELLOW}Entropía promedio durante la generación: {avg_entropy:.4f}{Style.RESET_ALL}")
        
        return generated_text, probability_history

# Celda 4: Funciones de entrenamiento y visualización
# Función para entrenar el modelo
def train_model(model, dataloader, epochs, learning_rate=0.001, processor=None, seed_text=None):
    """
    Entrena el modelo
    Args:
    ...
    for epoch in range(epochs):
        ...
        print(f"{Fore.GREEN}Época {epoch+1}/{epochs} completada en {epoch_time:.2f} segundos. Pérdida: {avg_loss:.4f}, Perplejidad: {perplexity:.4f}{Style.RESET_ALL}")
        
        # Generar muestra de texto si se proporcionan processor y seed_text
        if processor and seed_text and (epoch + 1) % max(1, epochs // 5) == 0:
            print(f"{Fore.CYAN}Generando muestra de texto en la época {epoch+1}...{Style.RESET_ALL}")
            model.eval()
            sample, _ = model.generate_text(processor, seed_text, length=100, temperature=1.0)
...
def visualize_prediction_distribution(model, processor, seed_text, temperatures=[0.5, 1.0, 1.5]):
    """
    ...
    """
    print(f"{Fore.BLUE}Visualizando distribución de probabilidades para la semilla: '{seed_text}'{Style.RESET_ALL}")
    ...
    print(f"{Fore.GREEN}Visualización de distribución de probabilidades completada.{Style.RESET_ALL}")
...
def visualize_lstm_states(model, processor, text, seq_length):
    ...
    for layer in range(model.num_layers):
        ...
        # Crear heatmap
        sns.heatmap(hidden_state.reshape(1, -1), cmap='viridis', 
    ...
    sns.heatmap(output, cmap='coolwarm', 
...
def analyze_ngrams(original_text, generated_text, n=2, top_k=20):
    ...
    """
    print(f"{Fore.BLUE}Analizando {n}-gramas entre texto original y generado...{Style.RESET_ALL}")
    ...
    ngrams_orig_display = [f"'{ng}'" if not ng.isspace() else "'espacio'" for ng in ngrams_orig]
    ...
    ngrams_gen_display = [f"'{ng}'" if not ng.isspace() else "'espacio'" for ng in ngrams_gen]
    ...
    print(f"{Fore.CYAN}Similitud de Jaccard para {n}-gramas: {jaccard:.4f}{Style.RESET_ALL}")
    print(f"Interpretación: {jaccard:.2%} de los {n}-gramas son compartidos entre ambos textos")
...
def evaluate_text_quality(original_text, generated_text):
    ...
    print(f"{Fore.YELLOW}KL Divergence entre distribuciones de caracteres: {kl_div:.4f}{Style.RESET_ALL}")
    ...
    print(f"{Fore.YELLOW}Solapamiento de palabras más comunes: {overlap}/10 ({overlap/10:.0%}){Style.RESET_ALL}")
    ...
    if min_length > 0:
        ...
        correlation = np.corrcoef(orig_lengths[:min_length], gen_lengths[:min_length])[0,1]
    else:
        correlation = 0.0
...
def plot_generation_entropy(probability_history, processor):
    ...
    plt.axhline(y=avg_entropy, color='r', linestyle='--', 
    ...
    plt.axhline(y=avg_confidence, color='r', linestyle='--', 
...
def main():
    ...
    print(f"  Input Layer ({processor.vocab_size}) → LSTM Layer 1 ({hidden_size}) → LSTM Layer 2 ({hidden_size}) → Dropout ({0.2}) → Fully Connected ({processor.vocab_size}) → Softmax")
    
    print(f"\n{Fore.CYAN}INICIANDO ENTRENAMIENTO{Style.RESET_ALL}")
    seed_text = "en un lugar de la mancha"
    history = train_model(model, dataloader, epochs, learning_rate, processor, seed_text)
    ...
    for temp in temperatures:
        ...
        generated_text, prob_history = model.generate_text(processor, seed_text, length=500, temperature=temp)
    ...
    plt.plot(temps, scores, marker='o', linestyle='-', color='purple', linewidth=2, markersize=8)
    ...
    for n in range(1, 4):  # Analizar 1-gramas, 2-gramas y 3-gramas
        jaccard_scores[n] = analyze_ngrams(processor.text[:len(generated_texts[1.0])], 
                                          generated_texts[1.0], n=n)
    ...
    quality_metrics = evaluate_text_quality(processor.text[:len(generated_texts[1.0])], 
                                           generated_texts[1.0])
    ...
    for temp in temperatures:
        ...
        repetitiveness = repeated_trigrams / len(trigram_counter) if trigram_counter else 0
    ...
    for i, metric in enumerate(metrics):
        axes[i].plot(comparison_df['Temperatura'], comparison_df[metric], 
