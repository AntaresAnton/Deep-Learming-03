...
for epoch in range(EPOCHS):
    ...
    for (batch, (inp, tar)) in enumerate(train_dataset):
        ...
        if batch % 50 == 0:
            print(f'Época {epoch + 1} Batch {batch} Pérdida {train_loss.result():.4f} Precisión {train_accuracy.result():.4f}')
    ...
    print(f'Época {epoch + 1}: Pérdida {train_loss.result():.4f} Precisión {train_accuracy.result():.4f}')
    ...
    train_dataset = create_dataset(train_encoder_input, train_decoder_input, train_decoder_target, BATCH_SIZE)
    val_dataset = create_dataset(val_encoder_input, val_decoder_input, val_decoder_target, BATCH_SIZE)
...
# División de datos
train_enc_input, val_enc_input = train_test_split(encoder_input, test_size=0.2, random_state=42)
train_dec_input, val_dec_input = train_test_split(decoder_input, test_size=0.2, random_state=42)
train_dec_target, val_dec_target = train_test_split(decoder_target, test_size=0.2, random_state=42)
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training=None, look_ahead_mask=None, padding_mask=None):
        ...
        for i, dec_layer in enumerate(self.dec_layers):
            x, block1, block2 = dec_layer(x, enc_output, training=training,
...
class Transformer(tf.keras.Model):
    ...
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
        ...
        self.encoder = Encoder(num_layers, d_model, num_heads, dff,
                              input_vocab_size, max_seq_length, dropout_rate)

        self.decoder = Decoder(num_layers, d_model, num_heads, dff,
                              target_vocab_size, max_seq_length, dropout_rate)

        self.final_layer = tf.keras.layers.Dense(target_vocab_size)

    def call(self, inputs, training=None, enc_padding_mask=None,
        ...
        dec_output, dec_attention_weights = self.decoder(tar, enc_output, training=training,
...
transformer = Transformer(
    ...
    dropout_rate=CONFIG['dropout_rate'])        # ✅ CORRECTO
...
for epoch in range(CONFIG['epochs']):
    ...
    try:
        ...
        for batch, (enc_inp, dec_inp, dec_tar) in enumerate(train_dataset):
            ...
            if batch % 5 == 0:
                print(f"   Batch {batch + 1}/{train_batches} - "
...
def predict_response(input_text, max_length=15):
    ...
    try:
        ...
        if filtered_sequence:
            reverse_word_index = {v: k for k, v in preprocessor.tokenizer.word_index.items()}
            words = [reverse_word_index.get(token_id, '<unk>') for token_id in filtered_sequence]
            response = ' '.join(words)
...
if len(train_history['train_loss']) > 0:
    ...
    # Gráfica de pérdida
    plt.subplot(1, 3, 1)
    plt.plot(train_history['epochs'], train_history['train_loss'], label='Train Loss', marker='o')
    plt.plot(train_history['epochs'], train_history['val_loss'], label='Val Loss', marker='s')
    plt.title('Pérdida durante el Entrenamiento')
    plt.xlabel('Época')
    plt.ylabel('Pérdida')
    plt.legend()
    ...
    plt.subplot(1, 3, 2)
    plt.plot(train_history['epochs'], train_history['train_accuracy'], label='Train Acc', marker='o')
    plt.plot(train_history['epochs'], train_history['val_accuracy'], label='Val Acc', marker='s')
    plt.title('Precisión durante el Entrenamiento')
    plt.xlabel('Época')
    plt.ylabel('Precisión')
    plt.legend()
...
print("\n" + "="*60)
print("🎉 CHATBOT TRANSFORMER COMPLETADO")
print("="*60)
...
if len(train_history['train_loss']) > 0:
    ...
    print(f"   🔹 Tiempo total de entrenamiento: {sum(train_history.get('epoch_times', [0])):.2f}s")
    print(f"   🔹 Épocas completadas: {len(train_history['epochs'])}")
    print(f"   🔹 Mejor pérdida de validación: {min(train_history['val_loss']):.4f}")
    print(f"   🔹 Precisión final: {train_history['val_accuracy'][-1]:.1%}")
    print(f"   🔹 Tamaño del vocabulario: {preprocessor.vocab_size}")
    print(f"   🔹 Parámetros del modelo: ~{sum([tf.size(var) for var in transformer.trainable_variables]).numpy():,}")

print("\n🛠️ COMPONENTES DISPONIBLES:")
print("   ✅ Modelo Transformer entrenado")
print("   ✅ Preprocesador de texto")
...
print("   📝 predict_response(texto) - Generar respuesta")
print("   💬 interactive_chat() - Chat interactivo")
print("   💾 save_model() - Guardar modelo")

print("\n📚 ARCHIVOS GENERADOS:")
print("   🔹 chatbot_transformer_weights/ - Pesos del modelo")
print("   🔹 chatbot_tokenizer.pickle - Tokenizer")
print("   🔹 chatbot_config.json - Configuración")
print("   🔹 training_history.json - Historial de entrenamiento")

# =============================================================================
# 14. FUNCIONES ADICIONALES DE UTILIDAD
# =============================================================================
...
def load_model():
    ...
    try:
        ...
        # Cargar tokenizer
        with open('./chatbot_tokenizer.pickle', 'rb') as handle:
        ...
        model = Transformer(
            ...
            pe_input=config['max_seq_length'],
            pe_target=config['max_seq_length'],
            rate=config['dropout_rate']
        )

        # Cargar pesos
        model.load_weights('./chatbot_transformer_weights')
...
def analyze_attention(input_text, response):
    ...
    try:
        ...
        response_sequence = preprocessor.tokenizer.texts_to_sequences([response_with_tokens])
        response_padded = tf.keras.preprocessing.sequence.pad_sequences(
            response_sequence, maxlen=CONFIG['max_seq_length'], padding='post', value=0
        )

        decoder_input = tf.expand_dims(response_padded[0][:-1], 0)
        ...
        print(f"Error en análisis de atención: {str(e)}")
        return None
...
def get_model_info():
    ...
    print("📊 ARQUITECTURA:")
    print(f"   🔹 Tipo: Transformer Encoder-Decoder")
    print(f"   🔹 Capas: {CONFIG['num_layers']}")
    print(f"   🔹 Cabezas de atención: {CONFIG['num_heads']}")
    print(f"   🔹 Dimensión del modelo: {CONFIG['d_model']}")
    print(f"   🔹 Dimensión FFN: {CONFIG['dff']}")
    print(f"   🔹 Dropout: {CONFIG['dropout_rate']}")

    print("\n📚 DATOS:")
    print(f"   🔹 Vocabulario: {preprocessor.vocab_size} tokens")
    print(f"   🔹 Secuencia máxima: {CONFIG['max_seq_length']}")
    print(f"   🔹 Pares de entrenamiento: {len(conversation_data)}")
    print(f"   🔹 Batch size: {CONFIG['batch_size']}")

    print("\n⚙️ ENTRENAMIENTO:")
    print(f"   🔹 Optimizador: Adam con Custom Schedule")
    print(f"   🔹 Learning rate inicial: {CONFIG['learning_rate']}")
    print(f"   🔹 Warmup steps: {CONFIG['warmup_steps']}")
    print(f"   🔹 Función de pérdida: Sparse Categorical Crossentropy")
    print(f"   🔹 Early stopping: {CONFIG['patience']} épocas")

    if transformer.trainable_variables:
        total_params = sum([tf.size(var) for var in transformer.trainable_variables]).numpy()
        print(f"\n🔢 PARÁMETROS:")
...
def benchmark_model():
    ...
    test_phrases = [
        "hello", "how are you", "thank you", "goodbye", "help me"
    ]

    times = []
    for phrase in test_phrases:
        start_time = time.time()
        response = predict_response(phrase)
        end_time = time.time()
        response_time = end_time - start_time
        times.append(response_time)
        print(f"   '{phrase}' → {response_time:.3f}s")

    avg_time = np.mean(times)
    print(f"\n📊 Tiempo promedio de respuesta: {avg_time:.3f}s")
    print(f"📊 Respuestas por segundo: {1/avg_time:.1f}")
...
def create_sample_conversations():
    ...
    sample_conversations = [
        ...
        ["hi there", "what is your name", "thank you", "see you later"],
        ["good morning", "help me please", "thank you very much", "have a good day"],
        ["hey", "i am happy today", "that is wonderful", "bye"]
    ]

    for i, conversation in enumerate(sample_conversations, 1):
        print(f"\n🗣️ Conversación {i}:")
        for j, user_input in enumerate(conversation):
            if j % 2 == 0:  # Usuario
                response = predict_response(user_input)
                print(f"   🧑 Usuario: {user_input}")
                print(f"   🤖 Bot: {response}")
            else:  # Continuar conversación
                response = predict_response(user_input)
                print(f"   🧑 Usuario: {user_input}")
                print(f"   🤖 Bot: {response}")

# =============================================================================
# 15. EJECUTAR FUNCIONES FINALES
# =============================================================================
...
print("\n🎯 PRÓXIMOS PASOS:")
print("   1. Ejecuta interactive_chat() para probar el chatbot")
print("   2. Usa predict_response('tu mensaje') para respuestas individuales")
print("   3. Modifica CONFIG para experimentar con hiperparámetros")
print("   4. Agrega más datos de conversación para mejor rendimiento")

print("\n💡 CONSEJOS DE USO:")
print("   • El modelo funciona mejor con frases simples y claras")
print("   • Puedes entrenar más épocas para mejor rendimiento")
print("   • Experimenta con diferentes configuraciones")
print("   • Guarda el modelo regularmente durante entrenamientos largos")

print("\n🔧 COMANDOS ÚTILES:")
print("   interactive_chat()          # Iniciar chat")
print("   predict_response('hola')     # Respuesta individual")
print("   get_model_info()            # Información del modelo")
print("   benchmark_model()           # Probar velocidad")
print("   save_model()                # Guardar modelo")

# Función final de inicialización rápida
def quick_start():
    ...
    print("2. Probar respuestas individuales")
    print("3. Ver información del modelo")
    print("4. Hacer benchmark")
    print("5. Salir")
    ...
    while True:
        try:
            ...
            if choice == '1':
                ...
                while True:
                    user_input = input("\nEscribe tu mensaje (o 'back' para volver): ")
...
print(f"\n🎉 Para empezar rápidamente, ejecuta: quick_start()")

# Mostrar estado final
print(f"\n📊 ESTADO FINAL:")
print(f"   ✅ Modelo entrenado: {'Sí' if len(train_history.get('train_loss', [])) > 0 else 'No'}")
print(f"   ✅ Predicción disponible: Sí")
print(f"   ✅ Chat interactivo: Sí")
print(f"   ✅ Modelo guardado: Sí")
print(f"   ✅ Listo para usar: Sí")

print("\n" + "🎊" * 20)
print("¡CHATBOT TRANSFORMER COMPLETAMENTE FUNCIONAL!")
print("🎊" * 20)


interactive_chat()

predict_response("hello")
# Verifica que el scheduler funciona correctamente
test_schedule = CustomSchedule(CONFIG['d_model'], CONFIG['warmup_steps'])
print("✅ Test del scheduler:")
for step in [1, 10, 100, 1000]:
    lr = test_schedule(step)
    print(f"   Step {step}: Learning Rate = {lr:.6f}")
response = predict_response("hello there")
print(response)
quick_start()