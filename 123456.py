...
if torch.cuda.is_available():
    try:
        ...
        torch.cuda.set_per_process_memory_fraction(
            0.7
        )  # Usar 70% de la memoria disponible
...
def expand_dialogues(df):
    ...
    for _, row in df.iterrows():
        ...
        if len(dialogues) != len(acts) or len(dialogues) != len(emotions):
            print(
                f"Advertencia: Longitudes inconsistentes - diálogos: {len(dialogues)}, actos: {len(acts)}, emociones: {len(emotions)}"
            )
        ...
        for i in range(len(dialogues)):
            expanded_data.append(
                {"dialog": dialogues[i], "act": acts[i], "emotion": emotions[i]}
            )
...
act_to_idx = {
    act: idx for idx, act in enumerate(sorted(train_expanded["act"].unique()))
}
...
emotion_to_idx = {
    emotion: idx
    for idx, emotion in enumerate(sorted(train_expanded["emotion"].unique()))
}
...
class DialogDataset(Dataset):
    ...
    def __init__(
        self, data, word_to_idx, act_to_idx, emotion_to_idx, max_len, task="both"
    ):
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        if self.task == "act":
            ...
            return {"text": text_tensor, "act": act_tensor, "emotion": emotion_tensor}
...
train_act_dataset = DialogDataset(
    train_expanded, word_to_idx, act_to_idx, emotion_to_idx, max_len, task="act"
)
val_act_dataset = DialogDataset(
    val_expanded, word_to_idx, act_to_idx, emotion_to_idx, max_len, task="act"
)
test_act_dataset = DialogDataset(
    test_expanded, word_to_idx, act_to_idx, emotion_to_idx, max_len, task="act"
)

# Datasets para clasificación de emociones
train_emotion_dataset = DialogDataset(
    train_expanded, word_to_idx, act_to_idx, emotion_to_idx, max_len, task="emotion"
)
val_emotion_dataset = DialogDataset(
    val_expanded, word_to_idx, act_to_idx, emotion_to_idx, max_len, task="emotion"
)
test_emotion_dataset = DialogDataset(
    test_expanded, word_to_idx, act_to_idx, emotion_to_idx, max_len, task="emotion"
)
...
train_emotion_loader = DataLoader(
    train_emotion_dataset, batch_size=batch_size, shuffle=True
)
...
class SimpleRNN(nn.Module):
    ...
    def __init__(
        self, vocab_size, embedding_dim, hidden_dim, output_dim, n_layers=1, dropout=0.5
    ):
...
rnn_act = SimpleRNN(
    len(word_to_idx), embedding_dim, hidden_dim, len(act_to_idx), n_layers, dropout
)
optimizer = optim.Adam(rnn_act.parameters(), lr=learning_rate)
criterion = nn.CrossEntropyLoss()
rnn_act, rnn_act_history = train_model(
    rnn_act,
    train_act_loader,
    val_act_loader,
    optimizer,
...
act_results["SimpleRNN"] = evaluate_model(
    rnn_act, test_act_loader, criterion, idx_to_act
)
...
criterion = nn.CrossEntropyLoss()
lstm_act, lstm_act_history = train_model(
    lstm_act,
    train_act_loader,
    val_act_loader,
    optimizer,
...
criterion = nn.CrossEntropyLoss()
gru_act, gru_act_history = train_model(
    gru_act,
    train_act_loader,
    val_act_loader,
    optimizer,
...
transformer_act = TransformerEncoder(
    len(word_to_idx),
    embedding_dim,
    hidden_dim * 2,
    len(act_to_idx),
...
criterion = nn.CrossEntropyLoss()
transformer_act, transformer_act_history = train_model(
    transformer_act,
    train_act_loader,
    val_act_loader,
    optimizer,
...
act_results["Transformer"] = evaluate_model(
    transformer_act, test_act_loader, criterion, idx_to_act
)
...
rnn_emotion = SimpleRNN(
    len(word_to_idx), embedding_dim, hidden_dim, len(emotion_to_idx), n_layers, dropout
)
optimizer = optim.Adam(rnn_emotion.parameters(), lr=learning_rate)
criterion = nn.CrossEntropyLoss()
rnn_emotion, rnn_emotion_history = train_model(
    rnn_emotion,
    train_emotion_loader,
    val_emotion_loader,
    optimizer,
    criterion,
    n_epochs,
...
emotion_results["SimpleRNN"] = evaluate_model(
    rnn_emotion, test_emotion_loader, criterion, idx_to_emotion
)
...
criterion = nn.CrossEntropyLoss()
lstm_emotion, lstm_emotion_history = train_model(
    lstm_emotion,
    train_emotion_loader,
    val_emotion_loader,
    optimizer,
...
emotion_results["LSTM"] = evaluate_model(
    lstm_emotion, test_emotion_loader, criterion, idx_to_emotion
)
...
criterion = nn.CrossEntropyLoss()
gru_emotion, gru_emotion_history = train_model(
    gru_emotion,
    train_emotion_loader,
    val_emotion_loader,
    optimizer,
...
emotion_results["GRU"] = evaluate_model(
    gru_emotion, test_emotion_loader, criterion, idx_to_emotion
)
...
transformer_emotion = TransformerEncoder(
    len(word_to_idx),
    embedding_dim,
    hidden_dim * 2,
    len(emotion_to_idx),
...
criterion = nn.CrossEntropyLoss()
transformer_emotion, transformer_emotion_history = train_model(
    transformer_emotion,
    train_emotion_loader,
    val_emotion_loader,
    optimizer,
...
emotion_results["Transformer"] = evaluate_model(
    transformer_emotion, test_emotion_loader, criterion, idx_to_emotion
)
...
def hyperparameter_analysis(model_class, param_name, param_values, task="act"):
    ...
    # Resultados
    results = {"param_value": [], "val_loss": [], "val_acc": [], "val_f1": []}
    ...
    for value in param_values:
        ...
        if param_name == "hidden_dim":
            model = model_class(
                len(word_to_idx), embedding_dim, value, output_dim, n_layers, dropout
            )
        elif param_name == "n_layers":
            model = model_class(
                len(word_to_idx), embedding_dim, hidden_dim, output_dim, value, dropout
            )
        elif param_name == "dropout":
            model = model_class(
                len(word_to_idx), embedding_dim, hidden_dim, output_dim, n_layers, value
            )
        ...
        model, history = train_model(
            ...
            criterion,
            5,
            f"{model_class.__name__}_{param_name}_{value}",
            task,
        )
...
hidden_dim_results = hyperparameter_analysis(
    model_class, "hidden_dim", hidden_dim_values, "act"
)

# Analizar n_layers
n_layers_values = [1, 2, 3]
n_layers_results = hyperparameter_analysis(
    model_class, "n_layers", n_layers_values, "act"
)
...
hidden_dim_values = [64, 128, 256]
hidden_dim_results = hyperparameter_analysis(
    model_class, "hidden_dim", hidden_dim_values, "emotion"
)

# Analizar n_layers
n_layers_values = [1, 2, 3]
n_layers_results = hyperparameter_analysis(
    model_class, "n_layers", n_layers_values, "emotion"
)

# Analizar dropout
dropout_values = [0.1, 0.3, 0.5]
dropout_results = hyperparameter_analysis(
    model_class, "dropout", dropout_values, "emotion"
)
...
def demo_prediction(text):
    ...
    act = predict_dialog(
        text,
        eval(f"{best_act_model_name.lower()}_act"),
        word_to_idx,
        idx_to_act,
        max_len,
    )
    ...
    # Predecir emoción
    emotion = predict_dialog(
        text,
        eval(f"{best_emotion_model_name.lower()}_emotion"),
        word_to_idx,
        idx_to_emotion,
        max_len,
    )
...
def create_prediction_app():
    ...
    print(
        "Ingrese un texto para predecir el acto de diálogo y la emoción (o 'salir' para terminar)"
    )
...
print(
    f"1. El mejor modelo para clasificación de actos de diálogo es {best_act_model_name} con un F1-score de {best_act_f1:.4f}"
)
...
print(
    f"2. El mejor modelo para clasificación de emociones es {best_emotion_model_name} con un F1-score de {best_emotion_f1:.4f}"
)

# Comparación entre arquitecturas
print("3. Comparación entre arquitecturas:")
print(
    "   - RNN vs LSTM/GRU: Las arquitecturas LSTM y GRU generalmente superan a la RNN simple debido a su capacidad para manejar dependencias a largo plazo."
)
print(
    "   - LSTM vs GRU: GRU tiende a ser más eficiente computacionalmente, mientras que LSTM puede capturar relaciones más complejas."
)
print(
    "   - RNN/LSTM/GRU vs Transformer: Los transformers pueden capturar relaciones globales en el texto sin depender de la secuencialidad, lo que los hace efectivos para tareas de NLP."
)

# Impacto de hiperparámetros
print("4. Impacto de hiperparámetros:")
print(
    "   - Dimensión oculta: Aumentar la dimensión oculta generalmente mejora el rendimiento hasta cierto punto, después del cual puede causar sobreajuste."
)
print(
    "   - Número de capas: Más capas pueden capturar relaciones más complejas, pero también aumentan el riesgo de sobreajuste y problemas de gradientes."
)
print(
    "   - Dropout: Un valor óptimo de dropout ayuda a prevenir el sobreajuste, especialmente en modelos más complejos."
)

# Desafíos y limitaciones
print("5. Desafíos y limitaciones:")
print(
    "   - Desbalance de clases: Algunas clases están subrepresentadas, lo que afecta el rendimiento del modelo."
)
print(
    "   - Tamaño del conjunto de datos: Un conjunto de datos más grande podría mejorar el rendimiento, especialmente para modelos complejos como los transformers."
)
print(
    "   - Contexto limitado: Los modelos actuales no consideran el contexto completo de la conversación, solo enunciados individuales."
)
...
print(
    "   - Implementar modelos pre-entrenados como BERT o GPT para mejorar el rendimiento."
)
print(
    "   - Desarrollar un sistema end-to-end que combine la clasificación de actos y emociones."
)
...
with open("conclusiones.txt", "w") as f:
    ...
    f.write(
        "   - Explorar técnicas de aumento de datos para clases subrepresentadas.\n"
    )
    f.write(
        "   - Implementar modelos pre-entrenados como BERT o GPT para mejorar el rendimiento.\n"
    )
