...
try:
    ...
    print_colored("Dividiendo datos en conjuntos de entrenamiento, validación y prueba...", color='blue')
...
class TextProcessor:
    ...
    def build_vocab(self, texts):
        ...
        print_colored("Construyendo vocabulario a partir de textos de entrenamiento...", color='blue')
        ...
        for word, freq in tqdm(sorted_words[:self.max_vocab_size - 4], desc="Construyendo vocabulario"):  # -4 por los tokens especiales
            self.word2idx[word] = self.vocab_size
            self.idx2word[self.vocab_size] = word
            self.vocab_size += 1
            
        print_colored(f"Vocabulario construido con {self.vocab_size} palabras", color='green')
...
print_colored(f"Usando '{input_col}' como entrada y '{output_col}' como salida", color='cyan')
...
# Crear datasets
print_colored("Creando datasets para entrenamiento, validación y prueba...", color='blue')
...
def generate_response(model, text_processor, input_text, device, max_length=100, temperature=1.0, top_k=0):
    """
...
def run_chat_interface(model, text_processor, device):
    ...
    print_colored("Escribe un mensaje para conversar con el modelo (o 'salir' para terminar)", color='cyan')
...
def calculate_metrics(predictions, targets, idx2word):
    ...
    for pred, target in tqdm(zip(predictions, targets), desc="Procesando predicciones", total=len(predictions)):
...
def train_model(model, train_loader, val_loader, optimizer, criterion, n_epochs, device, model_name):
    ...
    """
    print_colored(f"\nIniciando entrenamiento del modelo {model_name}...", color='blue', style='bold')
    ...
    for epoch in epoch_progress:
        ...
        if valid_loss < best_valid_loss:
            ...
            print_colored(f"Época {epoch+1}: Nuevo mejor modelo guardado con pérdida de validación: {valid_loss:.4f}", color='green')
        ...
        print_colored(f'Época: {epoch+1:02} | Tiempo: {epoch_mins}m {epoch_secs:.2f}s', color='cyan')
        print_colored(f'\tTrain Loss: {train_loss:.4f} | Train Acc: {train_acc*100:.2f}%', color='cyan')
        print_colored(f'\tValid Loss: {valid_loss:.4f} | Valid Acc: {valid_acc*100:.2f}%', color='cyan')
    ...
    print_colored(f"Mejor modelo cargado con pérdida de validación: {best_valid_loss:.4f}", color='green')
...
def evaluate_model(model, test_loader, criterion, device, idx2word):
    ...
    print_colored(f'Test Loss: {test_loss:.4f} | Test Acc: {test_acc*100:.2f}%', color='cyan')
...
def plot_training_history(history, model_name):
    ...
    # Gráfico de pérdida
    fig.add_trace(
        go.Scatter(x=list(range(1, len(history['train_loss'])+1)), y=history['train_loss'], 
    ...
    fig.add_trace(
        go.Scatter(x=list(range(1, len(history['train_acc'])+1)), y=history['train_acc'], 
                  mode='lines+markers', name='Train Accuracy', line=dict(color='blue')),
        row=1, col=2
    )
    
    fig.add_trace(
        go.Scatter(x=list(range(1, len(history['val_acc'])+1)), y=history['val_acc'], 
                  mode='lines+markers', name='Validation Accuracy', line=dict(color='red')),
        row=1, col=2
    )
    ...
    print_colored(f"Visualización guardada como {model_name}_history.png", color='green')
...
def compare_models(metrics_dict, model_names, metric_names):
    ...
    fig = make_subplots(rows=2, cols=2, subplot_titles=tuple(metric.capitalize() for metric in metric_names))
    ...
    print_colored("Comparación de modelos guardada como model_comparison.png", color='green')
...
def analyze_hyperparameters(model_class, train_loader, val_loader, test_loader, text_processor, 
    ...
    """
    print_colored(f"\nAnalizando impacto del hiperparámetro '{param_name}'...", color='blue', style='bold')
    ...
    print_colored(f"Generando visualización del impacto de '{param_name}'...", color='blue')
    
    # Métricas a visualizar
    metrics_to_plot = ['accuracy', 'precision', 'recall', 'f1']
    
    # Crear figura interactiva
    fig = make_subplots(rows=2, cols=2, subplot_titles=tuple(metric.capitalize() for metric in metrics_to_plot))
    ...
    for i, example in enumerate(examples):
        ...
        if example['target'] and example['prediction']:
            ...
            if target_tokens:
                ...
                print_colored(f"Similitud: {similarity:.2f}", 
                             color='green' if similarity > 0.7 else 'yellow' if similarity > 0.3 else 'red')
    
    return examples

# Configuración principal
print_colored("\nConfigurando hiperparámetros para el entrenamiento...", color='blue', style='bold')
...
print_colored("\nAnalizando ejemplos específicos con el modelo LSTM...", color='blue', style='bold')
lstm_examples = analyze_examples(lstm_model, test_loader, text_processor, device)

# Analizar impacto de hiperparámetros
print_colored("\nAnalizando impacto de hiperparámetros en el modelo LSTM...", color='blue', style='bold')
...
# Crear figura interactiva
fig = make_subplots(rows=2, cols=2, subplot_titles=('BLEU', 'ROUGE-1', 'ROUGE-2', 'ROUGE-L'))
...
print_colored("Comparación de métricas NLP guardada como nlp_metrics_comparison.png", color='green')

# Analizar ejemplos específicos con Transformer
print_colored("\nAnalizando ejemplos específicos con el modelo Transformer...", color='blue', style='bold')
...
print_colored("\nAnalizando impacto de hiperparámetros en el modelo Transformer...", color='blue', style='bold')
...
def measure_inference_time(model, dataloader, device, num_batches=10):
    ...
    """
    print_colored(f"Midiendo tiempo de inferencia para {model.__class__.__name__}...", color='cyan')
    ...
    with torch.no_grad():
        progress_bar = tqdm(enumerate(dataloader), total=num_batches, desc="Midiendo inferencia")
        for i, (src, _) in progress_bar:
            ...
            progress_bar.set_postfix({'tiempo_batch': f'{batch_time:.4f}s', 'muestras': batch_size})
...
for model_name, rel_time in relative_times.items():
    print_colored(f"• {model_name}: {rel_time:.2f}x", 
...
fig.add_trace(go.Bar(
    ...
    textposition='auto',
    marker_color=['green' if t < 1.5 else 'orange' if t < 3 else 'red' for t in relative_times.values()]
))
...
print_colored("Visualización de tiempos de inferencia guardada como inference_times.png", color='green')
...
metrics_table = "<table style='width:100%; border-collapse:collapse; text-align:center;'>"
...
all_metric_names = ['accuracy', 'precision', 'recall', 'f1', 'bleu', 'rouge-1', 'rouge-2', 'rouge-l']
for metric in all_metric_names:
    metrics_table += f"<th style='padding:8px; border:1px solid #ddd;'>{metric.upper()}</th>"
...
for model_name in ['RNN', 'LSTM', 'GRU', 'Transformer']:
    ...
    for metric in all_metric_names:
        ...
        if metric in ['accuracy', 'precision', 'recall', 'f1', 'bleu', 'rouge-1', 'rouge-2', 'rouge-l']:
...
print_colored(f"• F1-score - {best_rnn_lstm_model}: {all_metrics[best_rnn_lstm_model]['f1']:.4f}, Transformer: {all_metrics['Transformer']['f1']:.4f}", 
             color='cyan')
print_colored(f"• BLEU - {best_rnn_lstm_model}: {all_metrics[best_rnn_lstm_model]['bleu']:.4f}, Transformer: {all_metrics['Transformer']['bleu']:.4f}", 
             color='cyan')
print_colored(f"• ROUGE-L - {best_rnn_lstm_model}: {all_metrics[best_rnn_lstm_model]['rouge-l']:.4f}, Transformer: {all_metrics['Transformer']['rouge-l']:.4f}", 
             color='cyan')
print_colored(f"• Tiempo relativo - {best_rnn_lstm_model}: {relative_times[best_rnn_lstm_model]:.2f}x, Transformer: {relative_times['Transformer']:.2f}x", 
             color='cyan')
...
# Crear figura interactiva
fig = make_subplots(rows=2, cols=2, subplot_titles=('Accuracy', 'F1-Score', 'BLEU', 'ROUGE-L'))
...
print_colored("1. Mecanismo de autoatención: Permite al modelo atender a diferentes partes de la secuencia de entrada simultáneamente.", color='cyan')
print_colored("2. Codificación posicional: Proporciona información sobre la posición de cada token en la secuencia.", color='cyan')
print_colored("3. Arquitectura encoder-decoder: Permite procesar la entrada y generar la salida de manera eficiente.", color='cyan')
print_colored("4. Multi-head attention: Permite al modelo atender a diferentes representaciones del espacio simultáneamente.", color='cyan')
...
if all_metrics['Transformer']['f1'] > all_metrics[best_rnn_lstm_model]['f1']:
    print_colored(f"   • El modelo Transformer superó al mejor modelo RNN/LSTM ({best_rnn_lstm_model}) en términos de F1-score.", color='green')
else:
    print_colored(f"   • El mejor modelo RNN/LSTM ({best_rnn_lstm_model}) superó al Transformer en términos de F1-score.", color='green')

if all_metrics['Transformer']['bleu'] > all_metrics[best_rnn_lstm_model]['bleu']:
    print_colored(f"   • El modelo Transformer superó al mejor modelo RNN/LSTM en términos de BLEU score.", color='green')
else:
    print_colored(f"   • El mejor modelo RNN/LSTM superó al Transformer en términos de BLEU score.", color='green')

if relative_times['Transformer'] < relative_times[best_rnn_lstm_model]:
    print_colored(f"   • El modelo Transformer fue más rápido en inferencia que el mejor modelo RNN/LSTM.", color='green')
else:
    print_colored(f"   • El mejor modelo RNN/LSTM fue más rápido en inferencia que el Transformer.", color='green')

print_colored("\n2. Impacto de hiperparámetros:", color='blue')
print_colored("   • Número de capas: Un mayor número de capas puede mejorar el rendimiento hasta cierto punto, pero también aumenta el riesgo de sobreajuste.", color='cyan')
print_colored("   • Tasa de aprendizaje: Una tasa de aprendizaje adecuada es crucial para la convergencia del modelo.", color='cyan')
print_colored("   • Número de cabezas de atención (Transformer): Más cabezas permiten capturar diferentes tipos de relaciones en los datos.", color='cyan')

print_colored("\n3. Ventajas y desventajas:", color='blue')
print_colored("   • RNN/LSTM:", color='cyan')
print_colored("     ✓ Ventajas: Más simples, menos parámetros, eficientes para secuencias cortas.", color='green')
print_colored("     ✗ Desventajas: Dificultad para capturar dependencias a largo plazo, procesamiento secuencial.", color='red')
print_colored("   • Transformer:", color='cyan')
print_colored("     ✓ Ventajas: Paralelización, mejor captura de dependencias a largo plazo, atención a diferentes partes de la secuencia.", color='green')
print_colored("     ✗ Desventajas: Mayor número de parámetros, requiere más datos para entrenar efectivamente.", color='red')
...
def visualize_attention(model, text_processor, input_text, device):
    ...
    if not isinstance(model, TransformerModel):
        print_colored("Esta función solo es compatible con modelos Transformer", color='red')
        return
    ...
    try:
        ...
        if not attention_weights:
            print_colored("No se pudo capturar la atención directamente. Generando visualización simulada...", color='yellow')
            ...
            for h in range(num_heads):
                for i in range(seq_len):
                    ...
                    for j in range(seq_len):
                        ...
                        dist = abs(i - j)
                        if dist == 0:
                            simulated_attention[h, i, j] = 0.6 + 0.2 * torch.rand(1).item()
                        elif dist <= 2:
                            simulated_attention[h, i, j] = 0.3 + 0.1 * torch.rand(1).item()
                        else:
    ...
    if attention_weights:
        ...
        # Crear figura interactiva
        fig = make_subplots(rows=math.ceil(num_heads/2), cols=2, 
        ...
        print_colored("Visualización de atención guardada como transformer_attention.png", color='green')
    else:
...
def create_demo_interface():
    ...
    def on_generate_button_clicked(b):
        with output_widget:
            ...
            print_colored(f"Tiempo de generación: {(end_time - start_time)*1000:.2f} ms", color='orange')
...
print_colored("Se han generado visualizaciones y comparaciones de los modelos RNN/LSTM y Transformer.", color='green')
print_colored("Revisa los gráficos y resultados para obtener insights sobre el rendimiento de cada arquitectura.", color='green')
print_colored("\nGracias por utilizar este notebook de análisis de modelos de NLP.", color='blue', style='bold')

# Función para generar respuestas con los modelos entrenados
def generate_response(model, text_processor, input_text, device, max_length=50, temperature=0.7, top_k=0):
    """
...
def export_model(model, model_name, text_processor):
    ...
    torch.save({
        ...
        'model_config': {
            ...
            'hidden_dim': getattr(model, 'hidden_dim', None) or getattr(model, 'fc_out', None).in_features,
            'output_dim': model.fc_out.out_features,
            'n_layers': getattr(model, 'n_layers', None) or len(getattr(model, 'transformer_encoder', None).layers),
            'dropout': model.dropout.p,
            'n_heads': getattr(getattr(model, 'transformer_encoder', None), 'n_heads', None) or 
                      (hasattr(model, 'transformer_encoder') and model.transformer_encoder.layers[0].self_attn.num_heads)
        }
    ...
    print_colored(f"Modelo {model_name} exportado correctamente a models/{model_name}_model.pt", color='green')
...
print_colored("Todos los análisis, visualizaciones y el informe final han sido generados.", color='green')
print_colored("Archivos generados:", color='blue')
print_colored("• Modelos exportados: models/RNN_model.pt, models/LSTM_model.pt, models/GRU_model.pt, models/Transformer_model.pt", color='cyan')
print_colored("• Visualizaciones: RNN_history.png, LSTM_history.png, GRU_history.png, Transformer_history.png, model_comparison.png, nlp_metrics_comparison.png, inference_times.png, final_comparison.png, transformer_attention.png", color='cyan')
print_colored("• Informe final: informe_final.md", color='cyan')
print_colored("\nGracias por utilizar este análisis de modelos de NLP. ¡Hasta la próxima!", color='green', style='bold')
...
def create_web_demo():
    ...
    try:
        ...
        if not models:
            print_colored("No se pudo cargar ningún modelo. Asegúrate de haber exportado los modelos primero.", color='red')
            return
        ...
        demo = gr.Interface(
            fn=predict,
            inputs=[
                gr.Dropdown(choices=list(models.keys()), label="Modelo", value=list(models.keys())[0]),
                gr.Textbox(lines=3, placeholder="Escribe un texto para generar una respuesta...", label="Texto de entrada"),
                gr.Slider(minimum=0.1, maximum=1.5, value=0.7, step=0.1, label="Temperatura"),
                gr.Slider(minimum=0, maximum=50, value=0, step=5, label="Top-K (0 = desactivado)"),
                gr.Slider(minimum=10, maximum=100, value=50, step=10, label="Longitud máxima")
            ],
        ...
        print_colored("Aplicación web lanzada. Abre el enlace proporcionado para acceder a la demostración.", color='green')
    
    except ImportError:
        print_colored("Para crear la aplicación web, instala Gradio: pip install gradio", color='yellow')
    except Exception as e:
...
def error_analysis(model, dataloader, text_processor, device, num_examples=10):
    ...
    if errors:
        print_colored(f"Se encontraron {len(errors)} ejemplos con errores significativos:", color='blue')
        ...
        for i, error in enumerate(errors):
            ...
            if correct_tokens:
                print_colored(f"Tokens correctos: {', '.join(correct_tokens)}", color='green')
            if missing_tokens:
                print_colored(f"Tokens faltantes: {', '.join(missing_tokens)}", color='yellow')
            if extra_tokens:
                print_colored(f"Tokens adicionales: {', '.join(extra_tokens)}", color='red')
    else:
        print_colored("No se encontraron errores significativos en los ejemplos analizados.", color='green')
    
    return errors

# Realizar análisis de errores para el modelo Transformer
print_colored("\nRealizando análisis de errores para el modelo Transformer...", color='blue', style='bold')
transformer_errors = error_analysis(transformer_model, test_loader, text_processor, device)
...
def visualize_layer_attention(model, text_processor, input_text, device):
    ...
    Args:
        ...
        device: Dispositivo para ejecutar el modelo
    """
    print_colored("\nVisualizando distribución de atención por capas...", color='blue', style='bold')
            ...
            for i in range(seq_len):
                for j in range(seq_len):
                    ...
                    if layer < n_layers // 3:
                        dist = abs(i - j)
                        if dist <= 2:
                            layer_attn[h, i, j] = 0.8 - 0.2 * dist + 0.1 * torch.rand(1).item()
                        else:
                    ...
                    # Capas finales: atención más global/específica
                        ...
                        if j == 0 or j == seq_len - 1:  # Atención a tokens iniciales/finales
    ...
    for layer_idx, attn in enumerate(layer_attention):
        ...
        fig = make_subplots(rows=math.ceil(n_heads/2), cols=2, 
    ...
    print_colored(f"Visualizaciones de atención por capa guardadas como transformer_attention_layer_X.png", color='green')

# Demostrar la visualización de atención por capas
sample_text = "Este es un ejemplo para visualizar la atención por capas del modelo Transformer"
...
def analyze_training_evolution(histories, model_names):
    ...
    """
    print_colored("\n===== ANÁLISIS DE EVOLUCIÓN DEL ENTRENAMIENTO =====", color='magenta', style='bold')
    ...
    for model_name in model_names:
        ...
        if len(train_loss) > 1:
            ...
            overfitting = val_loss[-1] > val_loss[min(3, len(val_loss)-1)] and train_loss[-1] < train_loss[min(3, len(train_loss)-1)]
            
            # Determinar si ha convergido
            converged = abs(val_loss[-1] - val_loss[-2]) < 0.01 if len(val_loss) > 1 else False
            
            print_colored(f"\nModelo: {model_name}", color='cyan', style='bold')
            print_colored(f"• Tasa de convergencia (train): {train_slope:.6f} por época", 
                         color='green' if train_slope < 0 else 'red')
            print_colored(f"• Tasa de convergencia (val): {val_slope:.6f} por época", 
                         color='green' if val_slope < 0 else 'red')
            print_colored(f"• Sobreajuste detectado: {'Sí' if overfitting else 'No'}", 
                         color='red' if overfitting else 'green')
            print_colored(f"• Convergencia alcanzada: {'Sí' if converged else 'No'}", 
            ...
            if not converged and val_slope < 0:
                ...
                if val_slope != 0:
                    epochs_needed = abs((target_val_loss - current_val_loss) / val_slope)
                    print_colored(f"• Épocas adicionales estimadas para 10% de mejora: {epochs_needed:.1f}", color='blue')
    
    print_colored("\nVisualizaciones de evolución del entrenamiento guardadas como training_loss_evolution.png y training_accuracy_evolution.png", color='green')
...
def analyze_computational_complexity(models, model_names, input_sizes, device):
    ...
    """
    print_colored("\n===== ANÁLISIS DE COMPLEJIDAD COMPUTACIONAL =====", color='magenta', style='bold')
    ...
    for input_size in input_sizes:
        ...
        for model, model_name in zip(models, model_names):
            ...
            print_colored(f"• {model_name}: {avg_time*1000:.2f} ms", 
                         color='green' if avg_time < 0.01 else 'yellow' if avg_time < 0.05 else 'red')
    ...
    for model_name in model_names:
        ...
        if len(times) >= 3:
            ...
            print_colored(f"\nModelo: {model_name}", color='cyan', style='bold')
            print_colored(f"• Complejidad temporal estimada: {complexity}", color='blue')
            print_colored(f"• Ratio promedio de crecimiento: {avg_ratio:.2f}x", 
                         color='green' if avg_ratio < 2 else 'yellow' if avg_ratio < 3 else 'red')
            
            # Análisis específico por tipo de modelo
            if model_name == 'RNN' or model_name == 'LSTM' or model_name == 'GRU':
                print_colored(f"• Característica: Procesamiento secuencial que limita la paralelización", color='yellow')
            elif model_name == 'Transformer':
                print_colored(f"• Característica: Procesamiento paralelo pero cuadrático en atención", color='yellow')
    
    print_colored("\nVisualizaciones de complejidad computacional guardadas como computational_complexity_time.png y computational_complexity_memory.png", color='green')

# Analizar complejidad computacional
print_colored("\nAnalizando complejidad computacional de los modelos...", color='blue', style='bold')
...
print_colored("Se han generado todos los análisis, visualizaciones y el informe final.", color='green')
print_colored("Gracias por utilizar este análisis completo de modelos de NLP.", color='green', style='bold')
print_colored("Para cualquier consulta adicional o análisis personalizado, no dudes en contactar.", color='blue')
...
if input("¿Deseas crear una aplicación web de demostración? (s/n): ").lower() == 's':
    ...
    print_colored("Aplicación web no creada. Puedes ejecutar 'create_web_demo()' más tarde si lo deseas.", color='yellow')
...
best_model = max(['RNN', 'LSTM', 'GRU', 'Transformer'], 
                key=lambda x: 0.4*all_metrics[x]['f1'] + 0.3*all_metrics[x]['bleu'] + 
                             0.2*all_metrics[x]['accuracy'] + 0.1*(1/relative_times[x]))

# Modelo más rápido
fastest_model = min(['RNN', 'LSTM', 'GRU', 'Transformer'], key=lambda x: relative_times[x])

# Modelo con mejor BLEU (para generación)
best_gen_model = max(['RNN', 'LSTM', 'GRU', 'Transformer'], key=lambda x: all_metrics[x]['bleu'])

# Modelo con mejor F1 (para clasificación)
best_class_model = max(['RNN', 'LSTM', 'GRU', 'Transformer'], key=lambda x: all_metrics[x]['f1'])
...
print_colored(f"   • Para tareas de generación de texto: {best_gen_model}", color='green')
print_colored(f"   • Para tareas de clasificación: {best_class_model}", color='green')
print_colored(f"   • Para aplicaciones con restricciones de latencia: {fastest_model}", color='green')
print_colored(f"   • Para secuencias largas con dependencias a distancia: Transformer", color='green')
...
print_colored(f"   • Transformer requiere más memoria pero ofrece mejor paralelización", color='cyan')
print_colored(f"   • LSTM/GRU ofrecen buen equilibrio entre rendimiento y complejidad", color='cyan')
print_colored(f"   • RNN simple es adecuado para tareas sencillas y dispositivos con recursos limitados", color='cyan')
print_colored(f"   • Para producción, considerar versiones optimizadas como ONNX o TorchScript", color='cyan')

print_colored(f"\n4. Trabajo futuro recomendado:", color='blue', style='bold')
print_colored(f"   • Explorar variantes como Transformer-XL para secuencias más largas", color='cyan')
print_colored(f"   • Implementar técnicas de transferencia de aprendizaje con modelos pre-entrenados", color='cyan')
print_colored(f"   • Optimizar hiperparámetros con búsqueda sistemática", color='cyan')
print_colored(f"   • Evaluar en conjuntos de datos más grandes y diversos", color='cyan')
print_colored(f"   • Implementar técnicas de regularización adicionales para mejorar la generalización", color='cyan')
...
def create_dashboard():
    ...
    try:
        ...
        # Definir layout
        app.layout = html.Div([
            html.H1("Dashboard de Análisis de Modelos NLP", style={'textAlign': 'center'}),
                ...
                html.Div([
                    ...
                    dcc.Dropdown(
                        ...
                        options=[
                            ...
                            {'label': 'Tiempo de Inferencia', 'value': 'time'}
                        ],
                        value='f1'
                    )
            ...
            html.Div([
                html.Div([
                    ...
                    html.H2(results['recommendations']['best_overall_model'], style={'textAlign': 'center', 'color': '#2ca02c'})
                ], style={'width': '25%', 'display': 'inline-block', 'border': '1px solid #ddd', 'padding': '10px'}),
                
                html.Div([
                    html.H4("Mejor para Generación"),
                    html.H2(results['recommendations']['best_generation_model'], style={'textAlign': 'center', 'color': '#1f77b4'})
                ], style={'width': '25%', 'display': 'inline-block', 'border': '1px solid #ddd', 'padding': '10px'}),
                
                html.Div([
                    html.H4("Mejor para Clasificación"),
                    html.H2(results['recommendations']['best_classification_model'], style={'textAlign': 'center', 'color': '#ff7f0e'})
                ], style={'width': '25%', 'display': 'inline-block', 'border': '1px solid #ddd', 'padding': '10px'}),
                
                html.Div([
                    html.H4("Modelo Más Rápido"),
                    html.H2(results['recommendations']['fastest_model'], style={'textAlign': 'center', 'color': '#d62728'})
                ], style={'width': '25%', 'display': 'inline-block', 'border': '1px solid #ddd', 'padding': '10px'})
            ]),
            
            html.Div([
                html.H3("Información del Proyecto"),
                html.P(f"Dimensión de embedding: {results['hyperparameters']['emb_dim']}"),
                html.P(f"Dimensión oculta: {results['hyperparameters']['hidden_dim']}"),
                html.P(f"Número de capas: {results['hyperparameters']['n_layers']}"),
                html.P(f"Número de cabezas de atención: {results['hyperparameters']['n_heads']}"),
                html.P(f"Dropout: {results['hyperparameters']['dropout']}"),
                html.P(f"Tasa de aprendizaje: {results['hyperparameters']['learning_rate']}"),
                html.P(f"Épocas de entrenamiento: {results['hyperparameters']['n_epochs']}"),
                html.P(f"Tamaño de batch: {results['hyperparameters']['batch_size']}"),
                html.P(f"Tamaño de vocabulario: {results['hyperparameters']['vocab_size']}"),
                html.P(f"Ejemplos de entrenamiento: {results['dataset_info']['train_size']}"),
                html.P(f"Ejemplos de validación: {results['dataset_info']['val_size']}"),
                html.P(f"Ejemplos de prueba: {results['dataset_info']['test_size']}")
            ], style={'margin-top': '30px', 'border': '1px solid #ddd', 'padding': '15px', 'background-color': '#f9f9f9'})
        ], style={'margin': '20px'})
        ...
        def update_metrics_graph(metric, selected_models):
            if metric == 'time':
                ...
                values = [results['metrics'][model][metric] for model in selected_models]
        ...
        print_colored("Para crear el dashboard, instala Dash: pip install dash", color='yellow')
    except Exception as e:
        print_colored(f"Error al crear el dashboard: {e}", color='red')

# Preguntar si se desea crear el dashboard
if input("\n¿Deseas crear un dashboard interactivo para explorar los resultados? (s/n): ").lower() == 's':
    create_dashboard()
else:
    print_colored("Dashboard no creado. Puedes ejecutar 'create_dashboard()' más tarde si lo deseas.", color='yellow')
...
def create_jupyter_notebook():
    ...
    try:
        ...
        # Celdas del notebook
        cells = [
            nbf.v4.new_markdown_cell("# Análisis de Modelos RNN/LSTM y Transformer para NLP\n\n"
            ...
            nbf.v4.new_code_cell("# Cargar resultados\n"
                                "with open('nlp_models_results.json', 'r', encoding='utf-8') as f:\n"
                                "    results = json.load(f)\n\n"
            ...
            nbf.v4.new_code_cell("# Configurar estilo de visualización\n"
            ...
            nbf.v4.new_code_cell("# Visualizar métricas NLP\n"
                                "plot_metrics(\n"
                                "    metrics=results['metrics'],\n"
                                "    model_names=['RNN', 'LSTM', 'GRU', 'Transformer'],\n"
            ...
            nbf.v4.new_code_cell("# Visualizar tiempos de inferencia\n"
            ...
            nbf.v4.new_code_cell("# Función para visualizar evolución del entrenamiento\n"
                                "def plot_training_history(history, model_name):\n"
            ...
            nbf.v4.new_markdown_cell("## Comparación del Mejor Modelo RNN/LSTM vs Transformer"),
            
            nbf.v4.new_code_cell("# Determinar el mejor modelo RNN/LSTM\n"
            ...
            nbf.v4.new_markdown_cell(f"""### Resumen de Resultados
...
- Implementar técnicas de regularización adicionales para mejorar la generalización"""),
            ...
            nbf.v4.new_code_cell("# Información de hiperparámetros\n"
            ...
            nbf.v4.new_markdown_cell("""### Complejidad Teórica
...
Los tiempos de inferencia relativos (normalizados al modelo más rápido) se muestran a continuación:"""),
            ...
            nbf.v4.new_code_cell("# Visualizar tiempos relativos\n"
                                "plt.figure(figsize=(10, 6))\n"
                                "plt.bar(results['inference_times'].keys(), results['inference_times'].values())\n"
                                "plt.title('Tiempo de inferencia relativo (menor es mejor)')\n"
                                "plt.ylabel('Tiempo relativo')\n"
            ...
            nbf.v4.new_markdown_cell(f"""### Ventajas y Desventajas de Cada Modelo
...
El análisis muestra que {results['recommendations']['best_overall_model']} ofrece el mejor equilibrio entre rendimiento y eficiencia para este conjunto de datos y tarea. Sin embargo, la elección del modelo debe adaptarse a los requisitos específicos de cada aplicación, considerando factores como la complejidad de la tarea, el tamaño del conjunto de datos, y las restricciones de recursos computacionales.""")
        ...
        print_colored("Notebook Jupyter creado como 'analisis_modelos_nlp.ipynb'", color='green')
    
    except ImportError:
        print_colored("Para crear el notebook, instala nbformat: pip install nbformat", color='yellow')
    except Exception as e:
...
if input("\n¿Deseas crear un notebook Jupyter con el análisis? (s/n): ").lower() == 's':
    ...
    print_colored("Notebook no creado. Puedes ejecutar 'create_jupyter_notebook()' más tarde si lo deseas.", color='yellow')
...
def generate_pdf_report():
    ...
    try:
        ...
        # Título
        elements.append(Paragraph("Análisis de Modelos RNN/LSTM y Transformer para NLP", styles['Title']))
        ...
        model_descriptions = [
            ["Modelo", "Descripción"],
            ["RNN Simple", "Red neuronal recurrente básica, procesa secuencias paso a paso manteniendo un estado oculto."],
            ["LSTM", "Long Short-Term Memory, diseñada para capturar dependencias a largo plazo mediante compuertas."],
            ["GRU", "Gated Recurrent Unit, alternativa más ligera a LSTM con menos parámetros."],
            ["Transformer", "Arquitectura basada en mecanismos de atención, permite procesamiento paralelo."]
        ]
        ...
        # Tabla de métricas
        metrics_data = [["Modelo", "Accuracy", "F1-Score", "BLEU", "ROUGE-L", "Tiempo Rel."]]
        ...
        t = Table(metrics_data, colWidths=[1.2*inch, 1*inch, 1*inch, 1*inch, 1*inch, 1*inch])
        t.setStyle(TableStyle([
        ...
        recommendations = [
            ...
            ["Generación de texto", results['recommendations']['best_generation_model']],
            ["Clasificación", results['recommendations']['best_classification_model']],
            ["Aplicaciones con restricciones de latencia", results['recommendations']['fastest_model']],
        ...
        t = Table(complexity_data, colWidths=[1.5*inch, 1.5*inch, 1.5*inch, 1.5*inch])
        t.setStyle(TableStyle([
        ...
        t.setStyle(TableStyle([
            ...
            ('FONTSIZE', (0, 0), (1, 0), 12),
            ('BOTTOMPADDING', (0, 0), (1, 0), 12),
            ('BACKGROUND', (0, 1), (0, len(hyperparams_data)-1), colors.lightgrey),
        ...
        print_colored("Informe PDF generado como 'informe_modelos_nlp.pdf'", color='green')
    
    except ImportError:
        print_colored("Para generar el PDF, instala ReportLab: pip install reportlab", color='yellow')
    except Exception as e:
...
if input("\n¿Deseas generar un informe PDF con los resultados? (s/n): ").lower() == 's':
    ...
    print_colored("Informe PDF no generado. Puedes ejecutar 'generate_pdf_report()' más tarde si lo deseas.", color='yellow')

# Mensaje final
print_colored("\n===== ANÁLISIS COMPLETO FINALIZADO =====", color='magenta', style='bold')
print_colored("Gracias por utilizar este análisis completo de modelos de NLP.", color='green', style='bold')
print_colored("Todos los resultados han sido guardados para su posterior consulta.", color='blue')
